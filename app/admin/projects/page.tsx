...
export default function AdminProjects() {
  ...
  const addTechStack = () => {
    setFormData({
      ...formData,
      techStack: [...formData.techStack, { name: "", useCase: "" }],
    });
  };
  ...
  const removeTechStack = (index: number) => {
    const newStack = formData.techStack.filter(
  ...
  const removeInstallation = (index: number) => {
    const newInst = formData.installation.filter(
      (_: any, i: number) => i !== index,
    );
    setFormData({ ...formData, installation: newInst });
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(
          formData._id ? "Project updated successfully" : "New project created",
          "success",
        );
        setIsEditing(false);
  ...
  const editProject = (project: any) => {
    ...
    const cleanProject = {
      ...project,
      installation: Array.isArray(project.installation)
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-24 px-6 font-sans relative overflow-hidden">
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex justify-between items-center mb-10 border-b border-white/10 pb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-transparent bg-clip-text bg-linear-to-r from-white to-gray-500">
              Admin // Projects
            </h1>
          </div>
          {!isEditing && (
        ...
        {isEditing ? (
          <form
            ...
            className="bg-[#0a0a0a] p-8 rounded-xl border border-white/10 space-y-6 shadow-2xl relative animate-in fade-in slide-in-from-bottom-4"
          >
            ...
            <div className="space-y-4 bg-white/5 p-5 rounded-lg border border-white/10">
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  ...
                  className="flex items-center gap-2 text-xs bg-cyan-900/40 text-cyan-300 border border-cyan-500/30 px-3 py-1.5 rounded hover:bg-cyan-500 hover:text-black transition-colors"
                >
              ...
              {(!formData.installation ||
                formData.installation.length === 0) && (
                <div className="text-center py-6 text-gray-600 text-xs font-mono border border-dashed border-white/10 rounded bg-black/20">
                  // No installation protocols initialized
                </div>
              )}
              ...
              <div className="space-y-3">
                {formData.installation?.map((step: any, idx: number) => (
                  <div
                    key={idx}
                    className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center bg-black/40 p-3 rounded border border-white/5 hover:border-white/20 transition-colors"
                  >
                    {/* 1. Index Number */}
                    ...
                    <div className="col-span-12 md:col-span-4">
                      <label className="block md:hidden text-[10px] text-gray-500 mb-1">
                        STEP TITLE
                      </label>
                      <input
                        value={step.title}
                        onChange={(e) =>
                          updateInstallation(idx, "title", e.target.value)
                        }
                        placeholder="e.g. Clone Repository"
                        className="input-field w-full text-sm font-bold text-gray-300 placeholder-gray-700"
                      />
                    </div>

                    {/* 3. Command Input (The Terminal Part) */}
                    <div className="col-span-11 md:col-span-6 relative group">
                      <label className="block md:hidden text-[10px] text-gray-500 mb-1">
                        COMMAND
                      </label>
                      <div className="relative">
                        <Terminal
                          size={14}
                          className="absolute left-3 top-1/2 -translate-y-1/2 text-cyan-700 group-focus-within:text-cyan-400 transition-colors"
                        />
                        <input
                          value={step.command}
                          onChange={(e) =>
                            updateInstallation(idx, "command", e.target.value)
                          }
                          placeholder="npm install..."
                          className="input-field w-full pl-9 font-mono text-cyan-500 text-sm bg-black/50 border-white/10 focus:border-cyan-500/50"
                        />
                      </div>
                    </div>
              ...
              {formData.techStack.map((tech: any, idx: number) => (
                <div key={idx} className="flex gap-2 items-center">
                  ...
                  <input
                    value={tech.useCase}
                    onChange={(e) =>
                      updateTechStack(idx, "useCase", e.target.value)
                    }
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                onBlur={(e) =>
                  setFormData({
                    ...formData,
                    howItWorks: e.target.value.split("|").map((s) => s.trim()),
                  })
                }
            ...
            <button
              type="submit"
              className="w-full bg-cyan-600 hover:bg-cyan-500 py-4 rounded font-bold text-white transition-all hover:tracking-widest shadow-lg shadow-cyan-900/20"
            >
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 border-2 border-dashed border-white/10 rounded-2xl bg-[#0a0a0a]/50 text-center animate-in fade-in slide-in-from-bottom-4">
                ...
                <p className="text-gray-400 mb-8 max-w-sm font-light">
                  No active projects were found in the database. Initialize your
                  first project to display it on the portfolio.
                </p>
                <button
                  onClick={() => setIsEditing(true)}
              ...
              <div className="grid gap-4">
                {projects.map((p) => (
                  <div
                    key={p._id}
                    className="bg-[#0a0a0a] border border-white/10 p-5 rounded-lg flex justify-between items-center group hover:border-cyan-500/50 hover:bg-white/5 transition-all"
                  >
      ...
      <div
        className={`fixed bottom-6 right-6 z-50 transform transition-all duration-500 ease-in-out ${toast.show ? "translate-y-0 opacity-100" : "translate-y-12 opacity-0"}`}
      >
        <div
          className={`flex items-center gap-3 px-6 py-4 rounded-lg border shadow-2xl backdrop-blur-md min-w-[300px] ${
            toast.type === "success"
